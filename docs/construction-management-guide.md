...
**対象資格**: 1級・2級施工管理技士（建築、土木、電気工事、管工事、造園、建設機械）
...
例:
```bash
python scripts/upload_audio_cm.py "2025-01-15_CA001_client-call-001.m4a"
```

#### 方法B: 手動で情報を指定

```bash
python scripts/upload_audio_cm.py audio.m4a \
  --ca-id CA001 \
  --date 2025-01-15 \
...
```bash
# 音声ファイルの一覧を確認
ls -la data/audio_cm/pending/
```

### 書き起こしファイルを再紐付けしたい

既存の紐付けを上書きする場合は、再度 `link_transcript_cm.py` を実行してください。

### メタデータを確認したい

```bash
cat data/audio_cm/audio_metadata.json
```
